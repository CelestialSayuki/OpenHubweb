document.addEventListener('DOMContentLoaded', () => {
  ...
  let systemInWindowedMode = false;
  ...
  async function createWindow(pageUrl, titleText) {
    ...
    const isReplacingMaximized = maximizedWindow !== null;

    if (isReplacingMaximized && maximizedWindow.url !== pageUrl) {
      closeWindowWithAnimation(maximizedWindow.data.element, maximizedWindow.url);
    }

    const shouldMaximizeOnOpen = (openWindows.size === 0) || isReplacingMaximized;
    ...
    if (openWindows.has(pageUrl)) {
      ...
      if (windowData.state === 'minimized') {
        if (shouldMaximizeOnOpen) {
          windowData.element.classList.add('is-maximized');
        }
    ...
    try {
      if (isComparisonPage) {
        ...
        doc.head.querySelectorAll('link[rel="stylesheet"]').forEach(linkNode => {
          ...
          const elementId = `dynamic-style-${absoluteUrl}`;
          if (!document.getElementById(elementId)) {
            const newLink = document.createElement('link');
            newLink.id = elementId;
            newLink.rel = 'stylesheet';
            newLink.href = absoluteUrl;
            newLink.setAttribute('data-dynamic-style-for', windowData.id);
        ...
        doc.head.querySelectorAll('style').forEach(styleNode => {
          ...
          newStyle.setAttribute('data-dynamic-style-for', windowData.id);
          document.head.appendChild(newStyle);
        });

        const scriptsToLoad = Array.from(doc.querySelectorAll('script'));
        for (const scriptNode of scriptsToLoad) {
          const newScript = document.createElement('script');
          for (const attr of scriptNode.attributes) {
            newScript.setAttribute(attr.name, attr.value);
          }
          if (scriptNode.src) {
            const absoluteUrl = new URL(scriptNode.src, response.url).href;
      ...
      if (shouldMaximizeOnOpen) {
        if (isReplacingMaximized) {
          const windowEl = openWindows.get(pageUrl).element;
          windowEl.style.transition = 'none';
          toggleMaximize(pageUrl);
          setTimeout(() => {
            windowEl.style.transition = '';
      ...
      if (!aWindowIsClosing) {
        for (const [otherUrl, otherData] of openWindows.entries()) {
          if (otherUrl !== pageUrl && otherData.state === 'open') {
            closeWindowWithAnimation(otherData.element, otherUrl);
          }
        }
      }
